...
describe('Health endpoints (e2e)', () => {
  ...
  it('reports ready when PostgreSQL is reachable and tolerates optional Redis', async () => {
    ...
    expect(response.body).toEqual(
      expect.objectContaining({ status: 'ok', checks: { database: 'ok', redis: 'degraded' } }),
    )
  })
  ...
  it('returns 503 when PostgreSQL is unavailable', async () => {
    ...
    expect(response.body).toEqual(
      expect.objectContaining({ status: 'unavailable', checks: { database: 'unavailable', redis: 'degraded' } }),
    )
  })
})
