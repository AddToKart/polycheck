...
import { ArrayMaxSize, ArrayMinSize, IsArray, IsBoolean, IsDateString, IsIn, IsInt, IsNumber, IsOptional, IsString, Max, Min, MinLength, ValidateNested } from 'class-validator'
...
export class CreateBulkSessionsDto {
  ...
  @IsArray() @ArrayMinSize(1) @ArrayMaxSize(7) @IsIn(['Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat', 'Sun'], { each: true }) daysOfWeek: string[]
  @IsString() startTime: string
  @IsString() endTime: string
  @IsOptional() @IsString() room?: string
  @Type(() => Number) @IsInt() @Min(1) @Max(180) qrValidityMinutes: number
