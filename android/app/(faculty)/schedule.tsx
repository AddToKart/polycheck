    ...
    <SafeAreaView style={{ flex: 1, backgroundColor: isDark ? '#0B0B0E' : '#F7F6F6' }}>
      ...
      <View className="px-4 pt-2">
        <View className="flex-row rounded-2xl bg-zinc-200/70 p-1 dark:bg-white/5">
          {(['month', 'week'] as const).map((mode) => (
            <Pressable
              ...
              className={`min-h-11 flex-1 items-center justify-center rounded-xl ${viewMode === mode ? 'bg-maroon dark:bg-golden' : ''}`}
            >
              <Text className={`font-sans-bold text-xs capitalize ${viewMode === mode ? 'text-white dark:text-maroon-dark' : 'text-muted dark:text-zinc-400'}`}>{mode}</Text>
            </Pressable>
          ))}
        </View>
        ...
        <View className="mb-2 flex-row flex-wrap justify-center gap-x-4 gap-y-2">
          {[
            { label: 'Scheduled', color: 'bg-zinc-300 dark:bg-zinc-600' },
            { label: 'Session', color: 'bg-maroon dark:bg-golden' },
            { label: 'Active', color: 'bg-emerald-500' },
            { label: 'Moved', color: 'bg-zinc-500' },
          ].map((item) => (
            <View key={item.label} className="flex-row items-center gap-1.5">
              <View className={`h-2 w-2 rounded-full ${item.color}`} />
              <Text className="font-sans-bold text-[9px] uppercase tracking-[.6px] text-muted dark:text-zinc-500">{item.label}</Text>
            </View>
          ))}
        </View>
      </View>
        ...
        {viewMode === 'month' && selectedDay ? (
          <>
            ...
            <View className="gap-3">
              {selectedEvents.length ? [...selectedEvents].sort((a, b) => a.startTime.localeCompare(b.startTime)).map((event) => {
                const actionable = event.type === 'session' || event.status === 'moved'
                return (
                  <CampusCard key={event.id} onPress={actionable ? () => setSelectedEvent(event) : undefined} accessibilityLabel={actionable ? `Open ${event.subjectName}` : undefined} className="p-4">
                    <View className="flex-row items-center gap-4">
                      ...
                      <View className="flex-1">
                        <Text className="font-sans-bold text-sm text-ink dark:text-white">{event.status === 'moved' ? `${event.subjectName} · Moved` : event.type === 'schedule' ? 'No session activated' : event.subjectName}</Text>
                        <Text className="mt-1 font-sans text-xs text-muted dark:text-zinc-400">Sec {event.sectionName}{event.room ? ` · ${event.room}` : ''}</Text>
                      </View>
                      {actionable ? <MaterialIcons name="chevron-right" size={20} color="#746C6E" /> : null}
                    </View>
                  </CampusCard>
                )
              }) : (
                <CampusEmptyState icon="event-available" title="No classes this day" description="Choose another date or create a session from the Sessions tab." />
              )}
            </View>
          </>
        ) : null}
            ...
            <Pressable onPress={() => undefined} className="rounded-t-[36px] border-t-2 border-x border-maroon/20 bg-white px-5 pb-10 pt-4 shadow-2xl dark:border-golden/25 dark:bg-[#161214]">
              ...
              {selectedEvent.status === 'moved' && selectedEvent.rescheduledTo ? (
                <View className="mt-4 rounded-2xl border border-golden/50 bg-golden/10 p-4">
                  <Text className="font-sans-bold text-xs text-maroon dark:text-golden">Moved to a new class slot</Text>
                  <Text className="mt-2 font-sans text-xs leading-5 text-ink dark:text-zinc-200">{new Date(`${selectedEvent.rescheduledTo.date}T00:00:00`).toLocaleDateString('en-PH', { weekday: 'long', month: 'long', day: 'numeric' })} · {formatTime(selectedEvent.rescheduledTo.startTime)}–{formatTime(selectedEvent.rescheduledTo.endTime)}</Text>
                </View>
              ) : null}
              {selectedEvent.isRescheduled ? (
                <View className="mt-4 rounded-2xl bg-golden/10 p-4">
                  <Text className="font-sans-medium text-xs text-maroon dark:text-golden">This is a rescheduled make-up session.</Text>
                </View>
              ) : null}
              <View className="mt-4">
                <EventDetail icon="class" label="Section" value={`Sec ${selectedEvent.sectionName}`} />
                {selectedEvent.room ? <EventDetail icon="room" label="Room" value={selectedEvent.room} /> : null}
                <EventDetail icon="access-time" label="Time" value={`${formatTime(selectedEvent.startTime)}–${formatTime(selectedEvent.endTime)}`} />
                <EventDetail icon="today" label="Date" value={new Date(`${selectedEvent.date}T00:00:00`).toLocaleDateString('en-PH', { month: 'short', day: 'numeric', year: 'numeric' })} />
              </View>
              {counts ? (
                <View className="my-5 flex-row gap-2">
                  <View className="flex-1 rounded-2xl bg-emerald-500 p-3"><Text className="text-center font-sans-bold text-xs text-white">P {counts.present}</Text></View>
              ...
              <View className="mt-5 flex-row gap-3">
                <CampusButton className="flex-1" label="Close" variant="secondary" onPress={() => setSelectedEvent(null)} />
                {selectedEvent.type === 'session' ? (
                  <CampusButton className="flex-1" label="View session" icon="visibility" onPress={() => { const id = selectedEvent.id; setSelectedEvent(null); router.push(`/(faculty)/sessions/${id}`) }} />
                ) : null}
              </View>
            </Pressable>
          ) : null}
